...
app.controller("FeedbackReport", ["$scope", "$http", "GetService", "commonService", function ($scope, $http, GetService, commonService) {
    //#region DatePicker
    ...
    $scope.isBlogSuggestion = ($("#P-ID").val() === 'true' ? true : false);
    ...
    $scope.loadForwardedCases = function () {
        $("#loader").show();
        $scope.isBlogSuggestion = ($("#P-ID").val() === 'true' ? true : false);
        GetService.getFeedbackList(setPaginationOffset($scope.forwardedPagination), $scope.dateRang.str, $scope.dateRang.end, "/Report/GetFeedbackData", $scope.isBlogSuggestion).then(function (data) {
